...
async def endpoint():
    ...
    async with Endpoint.serve(config) as endpoint:
        ...
        await endpoint.connect_to_endpoints(
            ConnectionConfig.from_name(config.name),
        )
        yield endpoint
...
async def pair_of_endpoints():
    ...
    async with Endpoint.serve(config_1) as endpoint1:
        async with Endpoint.serve(config_2) as endpoint2:
...
async def triplet_of_endpoints():
    ...
    async with Endpoint.serve(config_1) as endpoint1:
        async with Endpoint.serve(config_2) as endpoint2:
            async with Endpoint.serve(config_3) as endpoint3:
